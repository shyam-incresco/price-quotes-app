...
export const DynamicTabs = ({
  ...
  return (
    <div
      className='flex flex-row gap-1 overflow-x-auto hide-scrollbar fixed mt-4 px-4' // Ensure it's overflow-x-auto for scrolling
      id='dynamicTabs'
      ref={tabsContainerRef}
    >
      {tabsData?.items?.map((tabItem: any, index: number) => {
        const isActive = tabItem?.path === currentPathName;

        return (
          <div
            key={index}
            ref={isActive ? activeTabRef : null} // Assign ref to active tab
          ...
          >
            ...
            {tabItem?.isClosable && (
              <div
                ...
                onClick={() => {
                  tabsData.deleteItem(index, navigation);
                  handleDeleteData();
                }}
              >
